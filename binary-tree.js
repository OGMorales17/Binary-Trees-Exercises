...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let higherThanLowerBond = currentVal > lowerBound;
      let shouldReassignClosest = currentVal < closest || closest === null;

      if (higherThanLowerBond && shouldReassignClosest) {
        closest = currentVal;
      }
  ...
  static deserialize(stringTree) {
    ...
    function buildTree() {
      // build a tree staring from the beginning of the array
      if (values.length) {
  ...
  lowestCommonAncestor(node1, node2, currentNode = this.root) {
    ...
    // base case 2: root is one of the target node
    ...
    // if neither left nor right is null, currentNodee is the ancestor
    if (left !== null && right !== null) return currentNode;

    // if one of the node is not null return it
